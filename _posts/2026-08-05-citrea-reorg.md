...
1. Only proceeds if the blockchain tip has changed since last check: `if new_tip != chain_state.current_tip {`

2. Goes backwards from the new tip, collecting block hashes for the last `finality_depth` blocks. Go from `i=1` and start with `current_hash` of new block height -1:
...
- This always matched (Hash matched && `pos`!=`i`: 0!=1 matched)
